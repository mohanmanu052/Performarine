...
class ManageDelegateApiProvider with ChangeNotifier
{
  Client client = Client();
  CommonModel? commonModel;

  String page = "manage_delegate_api_provider";

  Future<CommonModel?> manageDelegate(BuildContext context,
    ...
    debugPrint("MANAGE DELEGATE REQ BODY ${body}");

    Uri uri = Uri.https(Urls.baseUrl, Urls.manageDelegate);

    Utils.customPrint('MANAGE DELEGATE REQ $body');
    ...
    try {
      ...
      kReleaseMode ? null : Utils.customPrint('MANAGE DELEGATE : ' + response.body);
      kReleaseMode
          ? null
          : Utils.customPrint('MANAGE DELEGATE code : ' + response.statusCode.toString());
      Utils.customPrint('MANAGE DELEGATE code 1: $decodedData');
      ...
      kReleaseMode ? null : Utils.customPrint('error caught MANAGE DELEGATE :- $exception \n $s');

      CustomLogger().logWithFile(Level.error, "error caught MANAGE DELEGATE :- $exception \n $s -> $page");
