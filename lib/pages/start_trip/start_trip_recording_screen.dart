...
class _StartTripRecordingScreenState extends State<StartTripRecordingScreen>
  ...
  void initState() {
    ...
    checkPermissionsAndAutoConnectToDevice();

  }

  checkPermissionsAndAutoConnectToDevice() async
  {
    bool isNDPermDenied =
    await Permission
        .bluetoothConnect
  ...
  autoConnectToDevice() async {
    ...
    if(connectedDevicesList.isEmpty){
      ...
      // Show snackbar -> "Connected to <device_name> device."
      setState(() {
...
// >>>>>>> GOE-Sprint12-Base
  ...
  startWritingDataToDB(BuildContext bottomSheetContext) async {
    ...
    if (runningTrip!) {
      ...
      Utils.customPrint('CALLED FROM: ${widget.calledFrom}');
