        ...
        child: Stack(
          ...
          children: [
            SingleChildScrollView(
              child: Container(
                margin: EdgeInsets.only(bottom: displayHeight(context) * 0.09),
                child: Column(
                ...
                child: CommonButtons.getActionButton(
                    ...
                    onTap: () {
                      if (formKey.currentState!.validate()) {
                        CreateVessel addVesselData = CreateVessel();

                        addVesselData.name = nameController.text;
                        addVesselData.model = modelController.text;
                        addVesselData.builderName = builderNameController.text;
                        addVesselData.regNumber =
                            registrationNumberController.text;
                        addVesselData.mMSI = mmsiController.text;
                        addVesselData.engineType = selectedEngineType;
                        addVesselData.weight = int.parse(weightController.text);
    ...
    if (Platform.isAndroid) {
      ...
      if (isStoragePermissionGranted) {
        await selectImage(context, Colors.red,
          ...
          setState(() {
            pickFilePath.clear();
            pickFilePath.addAll(selectedImageFileList);
            kReleaseMode
                ? null
                : debugPrint('CAMERA FILE ${pickFilePath[0]!.path}');
    ...
    if (Platform.isAndroid) {
      ...
      if (isPermitted) {
        await selectImage(context, primaryColor,
            (List<File?> selectedImageFileList) {
          pickFilePath = selectedImageFileList;
          kReleaseMode
              ? null
              : debugPrint('CAMERA FILE ${pickFilePath[0]!.path}');
          setState(() {
