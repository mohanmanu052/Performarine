...
class _InviteDelegateState extends State<InviteDelegate> {
  ...
  checkIfDateIsSameAndSetTime() {
    int startYear = globalStartDate.year;
    int startMonth = globalStartDate.month;
    int startDay = globalStartDate.day;
    ...
    if (startYear == DateTime.now().year &&
      ...
      if (DateTime.now().minute < 10) {
        formattedMinute = DateTime.now().minute.toString().padLeft(2, '0');
      } else {
        formattedMinute = DateTime.now().minute.toString();
      }
      startTime =
      '$formattedHour : $formattedMinute ${DateTime.now().hour > 12 ? 'PM' : 'AM'}';
    }

    int endYear = globalEndDate.year;
    int endMonth = globalEndDate.month;
    int endDay = globalEndDate.day;
