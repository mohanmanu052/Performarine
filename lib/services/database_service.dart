...
class DatabaseService {
  ...
  Future<void> _onCreate(Database db, int version) async {
    ...
    await db.execute(
      'CREATE TABLE trips(id Text PRIMARY KEY,name Text,vesselId Text,vesselName Text, currentLoad TEXT, numberOfPassengers INTEGER DEFAULT 0, filePath Text,isSync INTEGER DEFAULT 0,'
      'tripStatus INTEGER  DEFAULT 0,deviceInfo Text, startPosition Text,endPosition Text,'
      ' createdAt TEXT,createdBy TEXT,updatedAt TEXT,duration TEXT,distance TEXT,speed TEXT,avgSpeed TEXT,isCloud INTEGER  DEFAULT 0)',
    );
    await db.execute(
...
Future<bool> checkIfTripExist(String tripId) async {
  ...
  Future<void> updateTripName(String tripId, String tripName) async {
    final db = await _databaseService.database;
   int count =  await db.rawUpdate(
        'UPDATE trips SET name = ?, isSync = ?, isCloud = ? WHERE id = ?', [tripName, 0, 0, tripId]);
  }
}
